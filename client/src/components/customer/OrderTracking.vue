  ...
  <Transition name="fade">
    ...
    >
      ...
      <main class="flex-1 overflow-y-auto p-5 pb-32">
        <div v-if="pedidoRastreado.status === 'cancelado'" class="bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-4 mb-5 text-center">
          <h4 class="font-bold text-neutral-900 dark:text-neutral-100 mb-1">Pedido cancelado</h4>
          <p class="text-sm text-neutral-600 dark:text-neutral-400">Seu pedido foi cancelado com sucesso.</p>
        </div>
        ...
        <!-- PIX Upload Section -->
        <div v-if="pedidoRastreado.paymentMethod === 'PIX' && pedidoRastreado.paymentStatus === 'pendente' && !pedidoRastreado.receiptUrl" class="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-xl p-5 mb-6 text-center shadow-sm">
          ...
          <template v-if="chavePixLoja && pixPayload">
            <p class="text-sm text-neutral-700 dark:text-neutral-300 mb-4">
              Escaneie o QR Code abaixo ou copie a chave PIX Copia e Cola para pagar <strong>{{ formatarMoeda(pedidoRastreado.total) }}</strong>. Depois, anexe o comprovante para liberarmos seu pedido.
            </p>

            <!-- QR Code PIX -->
            <div class="flex justify-center mb-4">
              <div class="bg-white p-3 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-800">
                <qrcode-vue :value="pixPayload" :size="180" level="M" />
              </div>
            </div>

            <div class="bg-white dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-lg p-3 font-mono text-neutral-900 dark:text-neutral-100 text-sm mb-4 break-all flex flex-col gap-2">
              <div class="text-left text-xs text-neutral-500 font-sans font-semibold">PIX Copia e Cola</div>
              <div class="max-h-20 overflow-y-auto text-left text-xs">{{ pixPayload }}</div>
              <button @click="copiarChave" class="mt-2 text-xs bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-400 font-bold px-3 py-2 rounded-md hover:bg-red-200 dark:hover:bg-red-900/60 transition flex items-center justify-center gap-1 w-full uppercase">
          ...
          <template v-else>
            <div class="py-8 flex flex-col items-center justify-center">
              <div class="w-8 h-8 border-4 border-red-200 border-t-red-600 rounded-full animate-spin mb-3"></div>
              <p class="text-sm text-neutral-500">Gerando código PIX...</p>
            </div>
          </template>
          
          <label class="block w-full py-3.5 bg-red-600 text-white rounded-lg text-sm font-semibold transition-all duration-200 hover:bg-red-700 cursor-pointer shadow-md active:scale-95 text-center relative mt-2">
            <span v-if="!uploadingReceipt" class="flex justify-center items-center gap-2"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg> Anexar Comprovante do PIX</span>
            <span v-else class="flex justify-center items-center gap-2"><div class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div> Enviando...</span>
            <input type="file" accept="image/*" class="hidden" @change="handleFileUpload" :disabled="uploadingReceipt" />
          </label>
        </div>
        
        <div v-else-if="pedidoRastreado.paymentMethod === 'PIX' && pedidoRastreado.receiptUrl && pedidoRastreado.paymentStatus === 'pendente'" class="bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-900/30 rounded-xl p-5 mb-6 text-center shadow-sm">
          <div class="w-12 h-12 bg-blue-100 dark:bg-blue-800/30 rounded-full flex items-center justify-center mx-auto mb-3 text-blue-600 dark:text-blue-400">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          </div>
          <h4 class="font-bold text-blue-800 dark:text-blue-300 mb-1">Comprovante Recebido!</h4>
...
const cancelarPedido = async () => {
  if (!props.pedidoRastreado?.trackingCode || !canCancelOrder.value || cancellingOrder.value) return;
